...
/* Dark mode puts the logo on a white panel so the navy lettering stays
   readable; light mode shows it bare (handled by the .wordmark-panel CSS rule
   in global.css). Hidden below lg by default. No React state → renders to
   static HTML with zero client JS. */

export function WordmarkLogo({ size = 'lg', base = '/' }: { size?: 'md' | 'lg'; base?: string }) {
  const h = size === 'lg' ? 'h-44 xl:h-52' : 'h-28 xl:h-32';
  const pad = size === 'lg' ? 'p-4' : 'p-3';
  return (
    <div className={`hidden lg:block shrink-0 ml-auto wordmark-panel ${pad}`}>
      <img
        src={`${base}cpi-logo.png`}
